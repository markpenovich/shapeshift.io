...
exports.shift = function (withdrawalAddress, pair, options, authToken, callback) {
  if (options.amount) {
    _shiftFixed(withdrawalAddress, pair, options.amount, options, authToken, callback)
  } else {
    _shift(withdrawalAddress, pair, options, authToken, callback)
  }
}

exports._shift = _shift
function _shift (withdrawalAddress, pair, options, authToken, callback) {
  var url = getBaseUrl() + '/shift'
  var payload = util.clone(options)
  payload.withdrawal = withdrawalAddress
  payload.pair = pair
  http.post(url, payload, authToken, function (err, data) {
...
function _shiftFixed (withdrawalAddress, pair, amount, options, authToken, callback) {
