...
exports.shift = function (apiKey, withdrawalAddress, pair, options, callback) {
  if (options.amount) {
    _shiftFixed(apiKey, withdrawalAddress, pair, options.amount, options, callback)
  } else {
    _shift(apiKey, withdrawalAddress, pair, options, callback)
  }
}

exports._shift = _shift
function _shift (apiKey, withdrawalAddress, pair, options, callback) {
  ...
  http.post(url, payload, function (err, data) {
    if (err) console.log(err) return callback(err, data)
    if (data.error) console.log(data.error) return callback(new Error(data.error), data)
    callback(null, data)
  })
}

exports._shiftFixed = _shiftFixed
function _shiftFixed (apiKey, withdrawalAddress, pair, amount, options, callback) {
